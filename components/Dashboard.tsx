        ...
        <div className="w-full h-full pt-4 relative">
             <div className="absolute inset-0 flex flex-col justify-between text-xs text-slate-300 pointer-events-none">
                 {[100, 75, 50, 25, 0].map(p => (
                     <div key={p} className="w-full border-b border-dashed border-slate-100 h-0"></div>
                 ))}
             </div>
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-48 overflow-visible">
                ...
                {data.map((d, i) => {
                     ...
                     const y = 100 - ((d.cost / maxVal) * 100);
                     return (
                         <circle key={i} cx={x} cy={y} r="1.5" fill="#10B981" stroke="white" strokeWidth="0.5" className="hover:r-2 transition-all" />
                     )
                })}
            </svg>
            <div className="flex justify-between mt-2">
                 {data.map((d, i) => (
                     <span key={i} className="text-xs text-slate-500">{d.name}</span>
                 ))}
            </div>
        </div>
    )
...
const Dashboard: React.FC<DashboardProps> = ({ project, allProjects, onSwitchProject, onCreateProject, onExport }) => {
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Dashboard</h1>
          <p className="text-slate-500 mt-2">Overview of {project.metadata.name}</p>
        </div>
        <button 
            onClick={onExport}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total Executions" value="12,450" sub="12%" icon={Activity} color="bg-blue-500" />
        <StatCard title="Avg Latency" value="1.2s" sub="4%" icon={Clock} color="bg-indigo-500" />
        <StatCard title="Token Usage" value="8.4M" sub="2.1%" icon={Zap} color="bg-amber-500" />
        <StatCard title="Cost" value="$432.20" sub="0.8%" icon={DollarSign} color="bg-emerald-500" />
      </div>
      ...
      {allProjects && (
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            <div className="flex justify-between items-center mb-4">
                ...
                {onCreateProject && (
                    <button onClick={onCreateProject} className="flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {allProjects.map(p => (
                    <div 
                        ...
                        className={`p-4 rounded-xl border transition-all cursor-pointer hover:shadow-md ${p.id === project.metadata.name ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-200' : 'bg-white border-slate-200 hover:border-indigo-300'}`}
                    >
                        <div className="flex justify-between items-start">
                            <div className="flex items-center space-x-3">
                                <div className={`p-2 rounded-lg ${p.id === project.metadata.name ? 'bg-indigo-200 text-indigo-700' : 'bg-slate-100 text-slate-500'}`}>
                                    <FileText size={18} />
                                </div>
                                <div>
                                    <h4 className={`font-semibold text-sm ${p.id === project.metadata.name ? 'text-indigo-900' : 'text-slate-800'}`}>{p.name}</h4>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col">
          <h3 className="text-lg font-bold text-slate-900 mb-2">Execution Volume</h3>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col">
          <h3 className="text-lg font-bold text-slate-900 mb-2">Token Consumption</h3>
          <div className="flex-1">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100">
            <h3 className="text-lg font-bold text-slate-900">Active Agents Status</h3>
        </div>
        <table className="w-full">
            <thead className="bg-slate-50">
                <tr>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Agent Name</th>
                    ...
                    <tr key={agent.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                             <div className="flex items-center">
                                <div className={`h-2.5 w-2.5 rounded-full mr-2 ${agent.executionStatus === 'running' ? 'bg-blue-500 animate-pulse' : agent.executionStatus === 'error' ? 'bg-red-500' : 'bg-emerald-500'}`}></div>
                                <span className={`font-medium ${agent.executionStatus === 'running' ? 'text-blue-700' : agent.executionStatus === 'error' ? 'text-red-700' : 'text-emerald-700'}`}>
                                    {agent.executionStatus === 'running' ? 'Running' : agent.executionStatus === 'error' ? 'Error' : 'Ready'}
                                </span>
                            </div>
                        </td>
                    </tr>
